...
4.  时间标记（硬性要求，必须严格遵守）：必须标记一个明确的、影响后续一组事件的宏观时间转变，输出一行单独的时间标记文本。
    - 格式必须是具体年月日，例如：2024年3月7日、2024年3月7日 下午、2024年3月7日到3月9日、2024年3月7日 深夜。
    - **严禁**出现任何模糊或占位符表达：某年、某月、某日、某年某月、某天、那天、当天、次日、翌日、几天后、不久后、后来、过了一段时间、X年X月、YYYY年MM月DD日 这类任何形式的占位写法都禁止。
    - 如果原文出现了明确日期，必须照搬原文日期。
    - 如果原文里完全没有日期线索，你必须自行**合理虚构**一个具体的年月日（建议落在 2020–2025 区间内），并在该段总结里保持该日期的一致性；后续如果再出现时间标记行，要让虚构时间线自洽递进（例如第一段写 2024年3月7日，下一段写 2024年3月8日 上午）。
    - 时间标记行不带序号和权重。脚本会自行处理这些时间标记。
...
4. 时间标记行严禁出现任何模糊或占位符：某年、某月、某日、某年某月、某天、那天、当天、次日、翌日、几天后、不久后、后来、过了一段时间、X年X月、YYYY年MM月DD日 等表达全部不允许。如果原文没有日期，自己合理虚构一个具体年月日（如 2024年3月7日 深夜）；绝不允许写"某年某月"。
你必须严格遵守输出格式。
...
---`;
    ...
    async function upsertSummaryEntry(lorebookName, chatId, startIdx0, endIdx0, summaryText) {
        ...
        // 强制刷新世界书面板：
        // 1) 重新拉取列表（保证下拉里的状态最新）
        // 2) 重新加载当前世界书编辑器（如果当前打开的就是这本，可立即看到新内容）
        // 3) 触发 WORLDINFO_UPDATED 事件给其他扩展
        try {
            if (typeof ctx.updateWorldInfoList === 'function') {
                await ctx.updateWorldInfoList();
            }
        } catch (e) { dbg('updateWorldInfoList failed', e); }
        try {
            if (typeof ctx.reloadWorldInfoEditor === 'function') {
                await ctx.reloadWorldInfoEditor(lorebookName, true);
            }
        } catch (e) { dbg('reloadWorldInfoEditor failed', e); }
        try {
            ctx.eventSource.emit(ctx.eventTypes.WORLDINFO_UPDATED, lorebookName, wi);
        } catch (_) {}
    }
