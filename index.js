...
4.  时间标记。必须输出一行单独的时间标记文本（不带序号和权重），格式为一个具体到「日」的日期表达，例如「7月1日」「2023年8月15日」「2023年8月15日到18日」「贞观三年九月初七」「建安五年三月十二」「光绪二十年腊月初三」等。强制规则：
    a. 严禁使用任何模糊表述，包括但不限于：某年、某月、某日、某天、某夜、次日、第二天、隔天、数日后、几天后、不久后、任务当天、当晚、那个夜晚、当时、此时等。出现上述词汇视为格式错误。
    b. 如果原文明确出现了日期或年份，必须直接采用原文中出现的日期。
    c. 如果原文中没有任何明确日期，必须根据上下文的时代背景（古代 / 近代 / 现代 / 近未来 / 架空奇幻 等）和情节走向，自行合理「创造」一个具体到日的日期，并写出来。例如：
       - 古代 / 仙侠 / 武侠：「贞观三年九月初七」「建安五年三月十二」「光绪二十年腊月初三」「永徽二年五月初一」
       - 近代 / 民国：「1923年10月14日」「民国十五年六月二日」
       - 现代：「2024年5月17日」「2023年12月8日」
       - 架空奇幻：「神历八二三年盛夏第十一日」之类亦可，但仍需具体到日
    d. 日期一旦在前文确立，后续总结必须保持时间线连贯（递进或合理跳跃），不得倒退、不得自相矛盾。
...
---`;
    ...
    async function upsertSummaryEntry(lorebookName, chatId, startIdx0, endIdx0, summaryText) {
        ...
        // 强制刷新世界书编辑器 UI，避免「必须点角色绑定的世界书才会刷新」的现象
        try {
            if (typeof ctx.reloadWorldInfoEditor === 'function') {
                await ctx.reloadWorldInfoEditor(lorebookName, true);
            }
        } catch (e) { dbg('reloadWorldInfoEditor 失败', e); }
        // 兜底：触发世界书选择器 / 列表 UI 刷新
        try {
